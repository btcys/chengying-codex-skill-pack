...
def main() -> int:
    ...
    if errors:
        print("帧芯开发工作流6.5校验失败：")
        for error in errors:
            print(f"- {error}")
        return 1

    print(f"帧芯开发工作流6.5静态包校验通过：{len(EXPECTED_SKILLS)}个Skill，manifest、界面版本、frontmatter、链接、命名、上下文预算和测试文件清单检查正常；运行脚本需另行烟测。")
    return 0
