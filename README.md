...
- 明确多线程判断时机：PM 第一轮只做预判；技术方案/架构影响面阶段正式决定 single-thread / multi-thread / defer；Handoff 前通过派发校验后才允许并行开发。
...
flowchart LR
  ...
  E --> ThreadDecision[多线程正式判断]
  ThreadDecision --> F[阶段计划 / QA 验证矩阵]
...
- 技术方案 / 架构影响面阶段才做正式判断：single-thread / multi-thread / defer。
