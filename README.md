...
- PHASE_PLAN 必须有计划版本号、执行顺序和每阶段三段式阶段版本号，例如 `0.0.1 / 0.1.0 / 0.4.5`；不能只用 `P0/P1/P2` 这类优先级标签代替阶段版本。每个阶段都要明确做什么、先做什么后做什么、允许/禁止范围、独立审计点、验收标准、阻塞验证和回滚方式。
...
flowchart LR
  ...
  E --> F[阶段计划 / QA 验证矩阵]
  F --> ThreadPlan[执行前线程方案]
  ThreadPlan --> Align[开发前对齐包 / Handoff 草案]
  Align --> G[Goal Prompt / 等待执行授权]
...
- 默认单线程，不默认多线程。
- PM 第一轮只能做多线程预判，不创建执行线程。
- 技术方案 / 架构影响面阶段只做可拆性分析和候选拆分，不最终决定线程数量。
- 阶段计划与验证方案完成后，在 Goal Prompt / Execution 前输出执行前线程方案，确定 single-thread / N threads / defer。
- Handoff 前必须完成派发校验，确认 Work ID、线程责任、数据/API 契约、验证矩阵、合并顺序和回滚策略后，才允许并行开发。
...
```text
请先整理开发前对齐包 / Handoff 草案，再基于已确认的 PRD、产品原型/交互草图、设计规范、技术方案/ADR、架构影响面、阶段开发计划、执行前线程方案、QA 验证矩阵和自动化模式，写出一段正式开发前 Goal Prompt，方便我确认或复制给 Codex 设置目标。只输出 Goal Prompt 不代表开始执行，Goal 也不替代 Handoff；如果复制给另一个 Codex，请连同开发前对齐包一起使用。必须等我明确说“按这个 Goal 执行 / 开始开发 / 进入 Execution”后，才登记 Work ID、冻结或补齐 PM Handoff 并进入执行。如果我暂不确认，请继续按阶段开发计划给我缺口、建议和下一步选择。
```
...
```text
按橙影工作流检查当前任务是否适合多线程。先做架构可拆性分析，等阶段计划与验证方案明确后，在 Goal Prompt / Execution 前输出执行前线程方案：开几个线程、每个线程的 Goal、修改范围、依赖、验收、合并、归档和回滚策略。
```
...
- PM Thread 不写代码，不初始化框架，不创建源码目录；必须按档位产出 PRD、产品原型/交互草图、设计规范、阶段开发计划、验证方案和开发前对齐包 / Handoff 草案，并在正式开发前输出一段式 Goal Prompt，等待用户明确执行授权。
...
- 用户不确认 Goal 或只是要求整理 Goal 时不进入执行线程；PM Thread 应继续分阶段沟通和给建议，不能把流程卡死。Goal 只是一段授权目标，真正执行必须依赖开发前对齐包 / Handoff。
