...
- PM 线程必须逼问出完整、可实现、可验收的 PRD。
- UI 项目必须逼问出完整设计规范；用户不确定时必须给方案选择。
...
- Product Design:image-to-code 只能在用户选定视觉目标、最终 Goal 确认并进入 Execution Thread 后使用。
...
- 进入正式开发前必须输出最终 Goal，并询问用户是否按该 Goal 计划执行；Goal 必须是一段面向用户确认的目标授权，并基于 PRD、设计规范、技术方案/ADR、架构影响面、阶段计划、QA 验证矩阵和自动化模式。
- Goal 只作为用户授权和目标确认，不替代 PM Handoff；其他线程必须以 Work ID、PM Handoff、阶段计划、验证矩阵和验收标准作为执行依据。
...
- 自动开发必须受控执行；确认 Goal 和自动化模式后，才允许在阶段内连续开发、测试、修复。
