...
const SCREEN_READY_FILE = path.join(STATE_DIR, 'screen-ready');
const WORKFLOW_VERSION = readWorkflowVersion();
...
<html>
<head><meta charset="utf-8"><title>帧芯可视化设计</title>
<style>
...
<body><!-- BRANDING --><h1>帧芯可视化设计</h1>
<p>正在等待开发任务展示设计方案……</p></body></html>`);
}
...
<body><h1>需要会话密钥</h1>
<p>请使用开发任务提供的完整URL，其中必须包含
<code>?key=&hellip;</code>。复制完整URL后重新打开。</p></body></html>`;

function bootstrapPage(key) {
  const jsonKey = JSON.stringify(String(key));
  return `<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>正在打开帧芯可视化设计</title></head>
<body>
<script>
...
function brandMarkup() {
  const version = escapeHtmlText(WORKFLOW_VERSION);
  return '<div class="brand"><span class="brand-copy">帧芯可视化设计 v' + version + '</span></div>';
}
...
function startServer() {
  ...
  if (fs.existsSync(SCREEN_READY_FILE)) fs.unlinkSync(SCREEN_READY_FILE);
  ...
  const watcher = fs.watch(CONTENT_DIR, (eventType, filename) => {
    ...
    debounceTimers.set(filename, setTimeout(() => {
      ...
      fs.writeFileSync(
        SCREEN_READY_FILE,
        JSON.stringify({ file: filename, timestamp: Date.now() }) + '\n',
        { mode: 0o600 }
      );
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' && !triedFallback) {
      if (tokenSource === 'env') {
        console.error('服务绑定失败：首选端口已被占用，且已显式设置BRAINSTORM_TOKEN；拒绝自动更换端口');
        process.exit(1);
      }
      triedFallback = true;
      PORT = randomPort();
      ...
      console.error('服务绑定失败：', err.message);
      process.exit(1);
    }
  });
  server.listen(PORT, HOST, onListen);
