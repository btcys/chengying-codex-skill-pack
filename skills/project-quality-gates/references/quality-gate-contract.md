...
承接项目确认的普通源文件和测试文件阈值及准确例外。6.5默认对既有超限文件增长报警，由Plan/Review判断局部修复或新增职责，不以行数自动阻断必要修复；新文件或原未超限文件仍执行硬上限。已有项目显式确认的更严门禁不能随插件升级擅自改写，调整时说明差异并取得确认。生成代码、第三方代码、迁移、快照和纯数据按项目规则排除，不要求无关重构。
