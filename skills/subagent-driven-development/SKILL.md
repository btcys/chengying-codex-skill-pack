...
description: 在帧芯开发工作流已有批准Plan、支持子智能体且分工有实际收益时，按明确交付派发开发并整合结果。
---
...
- Task可以组成职责清晰的功能批次；
- 主任务可以持续统筹同一个工作区；
- 当前运行时支持子智能体，且分工收益大于交接成本。

单一内聚任务、交接收益不足、没有子智能体能力或用户要求单线程时，使用 `$zhenxin-development-workflow:executing-plans`；不因工具可用而强制派发，也不额外请求分工批准。

## 不变原则

- 主任务不把整个Plan丢给一个代理后失去控制。
- 一个开发代理对应一个明确交付，可包含紧密相关的Task；交付完成后不追加新目标。续派仅补原任务上下文、修复原任务缺陷或处理对应Review反馈；新目标另派代理，小事可由主任务直接完成，不按微步骤拆代理。
- 子智能体只执行任务简报，不重新规划产品范围，也不得继续派发子智能体。
- 代理报告不能替代主任务对diff、测试证据和集成状态的检查。
- 不为每个Task安排独立Reviewer；整个Plan完成后做一次独立Review。
- 不自动commit、push、merge或创建worktree，不新增未经确认的依赖；按锁文件恢复已有依赖可在开发授权内执行，默认禁用安装脚本。生产写入及超出`Execution Mode`授权范围的外部副作用仍禁止。
...
等待优先使用完成通知或有界等待；需要决策、出现异常或用户询问时再查状态，不高频轮询或反复追问进度。主任务做不冲突的工作，不并行重做代理已承担的调查。工具读取按相关文件、行段和条数限范围；完整相关日志保存在现有证据位置，回传结论、关键错误和路径，不反复注入长日志，也不靠截断输出掩盖失败。

## 4. 接收与整合

每个开发代理返回后，主任务必须：

1. 读取报告和实际diff，不只看摘要；已核对且未变化的部分不重复调查，后续只检查新差异及其影响；
2. 确认没有越界修改、覆盖用户改动或增加未批准依赖；
3. 检查报告包含代码差异定位、测试文件、命令、结果和剩余问题；沿用现有报告和证据链接，不另建交付文档；
4. 根据代理状态处理：`NEEDS_CONTEXT`先补最小上下文，`BLOCKED`由主任务裁定，`DONE_WITH_CONCERNS`不能直接视为完成；
5. 按完成前验证规则复用未失效的定向证据，仅补缺失、失效检查和必要的整合验证，不因接收代理结果重跑同一套测试；
...
基础批次若定义了后续批次依赖的公共接口、Schema或状态合同，主任务核对diff和定向证据；证据缺失时先在授权内补齐，再进入依赖批次，确实无法补齐才报告阻塞，不默认增加独立Reviewer。

原任务实现或定向验证失败且仍有实质进展时，优先把明确反馈和证据发回原代理修复；无进展时交回主任务按 [返工与停止条件](../systematic-debugging/SKILL.md#返工与停止条件) 裁定，不以笼统的“继续”无限续派。更换代理只交接原范围、当前差异、有效证据、失败结论和剩余问题，不复制整段历史，也不清零返工记录。
