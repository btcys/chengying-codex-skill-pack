...
- Work ID 和活跃工作安排登记。
- `PHASE_PLAN.md` 当前计划版本、阶段 ID / 顺序和当前阶段，或 Lean 轻量任务单。
...
缺少当前档位的关键输入，不得验收通过。Phase Acceptance 不得只按“执行线程说完成了”验收，必须逐项对照 `PHASE_PLAN` 的计划版本、阶段 ID、独立审计点、独立验收标准、阻塞验证项和进入下一阶段条件。
...
- 当前阶段计划任务是否全部完成。
- 当前阶段是否匹配 `PHASE_PLAN` 版本和阶段 ID，没有拿旧计划或错误阶段验收。
