...
开发前声明必须说明当前对应 `PHASE_PLAN.md` 的计划版本、阶段版本（如 `0.0.1`）、阶段 ID 和执行顺序；Lean 快速路径说明对应轻量任务单即可。

## 执行中规则

- 一次只做一个任务。
- 严格按阶段开发计划的执行顺序执行，不得跳阶段或用 `P0/P1/P2` 优先级标签替代阶段版本。
...
- 涉及 UI、前端、可视化、图片/海报/日报卡、管理后台或用户可见页面时，截图是执行自检的强制项。内置浏览器失败、`file://` / `data:` 被策略拦截、Playwright 缺 Chromium、Chrome 权限不足或服务未启动时，不得直接提交“截图缺口”；必须先尝试项目 dev server / preview、`localhost` / `127.0.0.1` 静态服务、项目 e2e 截图命令或必要的外置 Chrome。需要启动服务、安装浏览器、运行 Docker、访问外置 Chrome 或更高权限时，先向用户请求一次明确且窄范围审批。
...
阻塞验证项未运行、失败、无证据或因缺工具/环境无法执行时，Execution Thread 不得标记为 `ready_for_acceptance`。截图、浏览器 smoke、Docker/部署启动等强制验证项必须先按恢复路径补齐，必要时请求用户审批；全部路径失败或审批被拒后，才允许标记为 `blocked` / `blocked_validation_missing`，列出阻塞验证缺口、缺少的工具/环境、已尝试路径、已请求审批和建议补验证方式。
...
- 截图恢复路径和审批记录：是否尝试内置浏览器、localhost / preview、临时静态服务、e2e 截图命令、外置 Chrome；是否请求过用户审批
