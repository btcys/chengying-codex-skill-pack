...
Execution Thread 必须在用户确认后启动，并且必须基于 PM Handoff。推荐新开 Execution Thread；如果当前环境没有线程工具，允许在当前线程显式声明“已切换到执行阶段”，但必须保留同等的 Work ID、Handoff、验证、验收和打回规则。
...
Execution Thread 的执行依据是 Work ID、PM Handoff、阶段计划、验证矩阵和验收标准。Goal 只证明用户授权了这轮开发，不能替代具体工作单。

如果当前上下文只有需求描述、PRD、Goal Draft 或 Goal Prompt，而没有开发前对齐包 / Handoff 草案，Execution Thread 必须退回 PM Thread 补齐上下文，不得自行推断修改范围、验收或验证方式。若已有 Handoff 但没有用户明确“按这个 Goal 执行 / 开始开发 / 进入 Execution”的确认记录，也必须退回 PM Thread 补齐授权，不得自行开始改代码。
...
开始自动开发前必须读取或生成 `VALIDATION.md`，并确认 QA 验证矩阵覆盖当前阶段。如果没有测试脚本，也没有可执行替代验证清单，不得开始自动开发。
...
开发前声明必须说明当前对应 `PHASE_PLAN.md` 的哪个阶段。
