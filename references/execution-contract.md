...
Execution Thread 必须在用户确认后由 PM Thread 派发启动，并且必须基于 PM Handoff 或 Lean 等价工作单。Level 2 / Standard 和 Level 3 / Enterprise 默认使用独立 Execution Thread 或已有执行线程；PM Thread 不得自行改代码。如果当前环境没有线程工具，允许在当前线程显式声明“已从 PM 切换到执行阶段”，但必须说明这是环境降级，并保留同等的 Work ID、Handoff 或等价工作单、验证、验收和打回规则。
...
如果任务影响公共组件、数据模型、API 契约、权限、状态管理、构建配置、部署或新增依赖，必须先确认技术方案；Lean 必须升级到 Standard。
