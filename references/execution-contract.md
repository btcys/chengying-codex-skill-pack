...
如果当前上下文只有需求描述、PRD、Goal Draft 或 Goal Prompt，而没有用户明确“按这个 Goal 执行 / 开始开发 / 进入 Execution”的确认记录，Execution Thread 必须退回 PM Thread 补齐授权，不得自行开始改代码。
