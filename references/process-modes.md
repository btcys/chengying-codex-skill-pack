...
| Standard | 正式小程序、网站、App、后台、中小型 SaaS | 完整 PRD、产品原型/交互草图、设计规范、技术方案、阶段计划和一段式 Goal 指令 | 20 到 45 分钟轻量竞品/开源扫描 | 核心 `docs/codex/` 文档 | Phase Acceptance 必须有；命中风险触发条件时必须 Code Review |
...
Lean UI Fix 可以按已有页面、明确截图或文字验收目标执行；只要求记录视觉依据来源、必须对照点、禁止偏离点和 before/after 截图证据，不需要重新走完整产品原型或视觉探索。只有纯文案、无可见 UI 变化、用户/PM 已在验收前明确标为非阻塞的情况，才可用轻量观察证据替代截图；否则仍按强制截图恢复路径补齐。

### Standard 默认压缩输出

Standard 保留完整主线，但默认先用压缩输出推进，不把完整模板一次性铺给用户。默认输出只需要覆盖：

- 全流程状态表：当前阶段、已有产物、缺口、下一步。
- 事实分层表：已确认 / 我的建议 / 待确认。
- 精简 Handoff：Work ID、目标、非目标、修改范围、禁止修改范围、验收方式、验证方式、回滚方式、线程判断。
- Goal 就绪判断：当前能否生成最终 Goal，不能时列出阻塞缺口。

完整模板是字段池，不是每轮都要填写的表单。只有以下情况才展开完整字段：

- 用户要求查看完整文档或准备写入仓库文档。
- 命中登录、权限、API、数据、部署、自动化、外部服务、安全、隐私或多线程风险。
- 即将冻结 `v1.0`、登记 Work ID、派发 Execution Thread、进入 Phase Acceptance / Requirement Compliance / Code Review。
- 项目升级为 Enterprise。
...
未命中以上条件时，Standard 可跳过 Code Review，但必须写明跳过原因和剩余风险。Enterprise 不适用跳过规则。
