...
- Handoff / `VALIDATION.md` / 阶段计划中的验证项已标明 `blocking` / `non-blocking`；没有分级时默认按 `blocking` 处理。
...
- 测试缺失且无法建立替代验证。
- 阻塞验证项未运行、失败、无证据，或因缺 Playwright/Chromium、Chrome 权限、Docker 命令、账号、外部服务、部署环境等工具/环境无法执行；截图、浏览器 smoke、Docker/部署启动等强制验证项在停止前必须先尝试恢复路径并请求必要审批，审批被拒或全部路径失败后才可作为最终阻塞。
