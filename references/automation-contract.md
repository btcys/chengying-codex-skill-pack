...
意思是：用户确认 Goal 前，PM 先形成开发前对齐包 / Handoff 草案；用户确认 Goal 后，PM 登记 Work ID 并冻结或补齐 PM Handoff 工作单。Codex 只能在该 Work ID、Handoff、阶段计划、修改范围和自动化模式内连续完成开发、自测、修复和文档更新；遇到门禁、风险、越界或需要产品判断时必须停止。
...
- 用户未确认 Goal 或自动化模式。
- 缺少开发前对齐包、Work ID、活跃工作安排登记或 PM Handoff 工作单。
- 需求、设计或验收标准不清。
- 修改范围需要扩大。
- 触碰禁止修改范围。
- 需要新增依赖。
