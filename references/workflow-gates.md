...
- `PHASE_PLAN` 没有计划版本号、阶段顺序、前后依赖、每阶段修改边界、独立审计点、独立验收标准、阻塞验证项和进入下一阶段条件时，不进入 Goal Prompt / Execution / Phase Acceptance。
