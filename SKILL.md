...
description: 对话式正式项目/长期项目 Codex 开发工作流。用于把目标、PRD、设计稿、代码仓库或正在开发的项目，交给 Codex 按任务强度选择短任务卡、PRD/设计/Dev Plan、最多 3-5 个互不依赖开发子线程、独立 Code Review、交付和自进化沉淀。强调先思考后动手、短文档、子线程 ownership 不重叠、需要时 UI 独立线程、AI 自主规划/开发/测试/修复/审查，用户只处理关键确认和高风险决策。
---
